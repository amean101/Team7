...
// Create indexes for performance optimization
    ...
    await d.execute('CREATE INDEX idx_items_found_time ON items(found_time DESC)');
    await d.execute('CREATE INDEX idx_items_category ON items(category_id)');
    await d.execute('CREATE INDEX idx_items_location ON items(found_location_id)');
...
// CRUD operations and queries below
  Future<int> insertItem(TItem it) async =>
      (await db).insert('items', it.toMap(), conflictAlgorithm: ConflictAlgorithm.replace);
...
// fetch items with optional filters
  Future<List<TItem>> items({
    ...
    final w = <String>[];
    final a = <Object?>[];
    if (status != null) { w.add('status=?'); a.add(status); }
    if (categoryId != null) { w.add('category_id=?'); a.add(categoryId); }
    if (locationId != null) { w.add('found_location_id=?'); a.add(locationId); }
    if (reportedBy != null) { w.add('reported_by=?'); a.add(reportedBy); }
    if (claimedBy != null) { w.add('claimed_by=?'); a.add(claimedBy); }
    ...
    final rows = await (await db).query(
      ...
      orderBy: newestFirst ? 'datetime(updated_at) DESC' : 'datetime(found_time) ASC',
    );
...
// get item counts by status
  Future<Map<String, int>> itemCounters() async {
    ...
    final total = await c('');
    final found = await c('WHERE status=?', ['found']);
    final claimed = await c('WHERE status=?', ['claimed']);
    final returned = await c('WHERE status=?', ['returned']);
    final archived = await c('WHERE status=?', ['archived']);
    return {'total': total, 'found': found, 'claimed': claimed, 'returned': returned, 'archived': archived};
  }
// upsert category
  Future<int> upsertCategory(String id, String name) async =>
      (await db).insert('categories', {'id': id, 'name': name}, conflictAlgorithm: ConflictAlgorithm.replace);
// upsert location
  Future<int> upsertLocation(String id, String name) async =>
      (await db).insert('locations', {'id': id, 'name': name}, conflictAlgorithm: ConflictAlgorithm.replace);
// upsert user
  Future<int> upsertUser({
    ...
    return (await db).insert(
      'users',
      {'id': id, 'name': name, 'email': email, 'role': role, 'created_at': createdAtIso},
...
// fetch photos for an item
  Future<List<Map<String, dynamic>>> itemPhotos(String itemId) async =>
      (await db).query('photos', where: 'item_id=?', whereArgs: [itemId], orderBy: 'datetime(created_at) DESC');
// create a claim
...
// fetch claims for an item
  Future<List<Map<String, dynamic>>> claimsForItem(String itemId) async =>
      (await db).query('claims', where: 'item_id=?', whereArgs: [itemId], orderBy: 'datetime(submitted_at) DESC');
// fetch claims for a user
  Future<List<Map<String, dynamic>>> claimsForUser(String userId) async =>
      (await db).query('claims', where: 'claimer_id=?', whereArgs: [userId], orderBy: 'datetime(submitted_at) DESC');
}
